...
static int findPID(pid pid, PCB ** pr);

// // static PCB processTable[MAX_PROCESSES]; // We store all of our proccesses info here
// int lastPID = 0;
...
static int findPID(pid pid, PCB **pr) {
    // if (pid < 0 || pid >= MAX_PROCESSES || processTable[pid].stackEnd == NULL) {
    //     return 0;
    // }
    // *pr = &processTable[pid];
    // return 1;
